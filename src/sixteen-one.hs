...
main = do
  ...
  putStrLn . show . sum . map getVSum $ packets
...
-- | gets the sum of the packet versions
getVSum :: Packet -> Int
getVSum p@(Literal {}) = fromBinaryToInt 0 . pversion $ p
getVSum p@(Operator {}) = (fromBinaryToInt 0 . pversion $ p) + (sum . map getVSum . plist $ p)
