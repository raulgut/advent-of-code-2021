...
main = do
  ...
  putStrLn . show . (\octopuses -> execState (process) (CState 0 0 empty octopuses)) . M.fromList $ [((i,j),ientry!!(j-1)) | i <- [1..length entries], let ientry = entries!!(i-1), j <- [1..length ientry]]

-- | 
process :: State CState ()
process 
  = do { state <- get 
       ; if (M.filter (==0) (grid state) == (grid state)) then
           return ()
         else
