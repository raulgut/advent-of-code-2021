...
main = do
  ...
  let (hpos, depth, aim) = execState (processInstructions instructions) (0,0,0) --- (hpos,depth,aim)
...
-- | process instruction and update horizontal position, depth and aim
processInstructions :: Num a => [Instruction a] -> State (a,a,a) ()
processInstructions [] = return ()
processInstructions ((Forward value):is) 
  = do { (hpos,depth, aim) <- get
       ; put (hpos + value, depth + (aim * value), aim)
       ; processInstructions is }
processInstructions ((Down value):is) 
  = do { (hpos,depth, aim) <- get
       ; put (hpos, depth, aim + value)
       ; processInstructions is }
