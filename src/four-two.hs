...
play boards (n:ns) = let newBoards = map (map (filter (/= n))) boards
                         restBoards = filterWinner newBoards
                     in if null restBoards then 
                          (*n) . foldl (+) 0 . map (foldl (+) 0) . take 5 . head $ newBoards
                        else 
                          play restBoards ns

-- | filter the winners
filterWinner :: [[[Int]]] -> [[[Int]]]
filterWinner [] = []
filterWinner (b:bs) = if (or . map null $ b) then 
                        filterWinner bs 
                      else 
                        b:(filterWinner bs)