...
main = do
  ...
  putStrLn . show . (\x -> sum [0..x]) . maximum . snd . foldl1 getMax . processValidYs 1 (snd . fst $ target) . snd $ target
...
getMax (i,vs) (j,[]) = (i,vs)
getMax (i,[]) (j,ws) = (j,ws)
getMax (i,vs) (j,ws) = if maximum ws > maximum vs then 
                         (j,ws)
                       else 
                         (i,vs) 