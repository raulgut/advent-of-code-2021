...
main = do
  ...
  case (parse parseScanners "" input) of
    ...
    Right (s:scanners) -> putStrLn . show . maximum . getManhattan . map fst . concat . M.elems . scans . analyzeSensors (Scanners (M.fromList [(fst s,[((0,0,0),snd s)])]) S.empty) S.empty . map (\(x,y) -> (x, getOrientations y)) $ scanners
...
getManhattan [] = []
getManhattan (x:xs) = let applyManhattan (x0,y0,z0) (x1,y1,z1) = (abs $ x0 - x1) + (abs $ y0 - y1) + (abs $z0 - z1)
                      in map (applyManhattan x) xs ++ getManhattan xs